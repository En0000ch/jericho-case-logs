...
{
    const OPTION_KEY = 'analyticswp_slt_license';
    ...
    public static function run_status_check($force_check = false)
    {
        ...
        if (strpos(ANALYTICSWP_WOO_SLT_INSTANCE, 'solidaffiliate') !== false) {
            return;
        };
        ...
        if (!$force_check && (time() < ($license_data['last_check'] + self::MINIMUM_TIME_BETWEEN_API_CHECKS_IN_SECONDS))) {
            return;
        } else {
